...
// In-memory "DB"
const profiles = new Map(); // key: handle
...
/** ---------- Utilities ---------- */
function makeUniqueHandle(preferred, fallbackName) {
...
function parseSelectedTags(params) {
    ...
    for (const t of selected) {
        if (allowed.has(t) && !filtered.includes(t))
            filtered.push(t);
...
function normalizeMakerId(input) {
...
function homePage(q, tag) {
    ...
    const cards = list
        ...
        ${top
            ? `<div class="muted small" style="margin-top:10px;">Top1: ${escapeHtml(top.title)} <span class="copy">(${escapeHtml(top.courseId)})</span></div>`
            : ""}
      </div>`;
    })
        .join("");
...
function profilePage(handle) {
    ...
    if (!p)
        return layout("見つかりません", `<p>プロフィールが見つかりません。<a href="/">トップへ</a></p>`);
    const top10 = p.top10
...
const server = http.createServer(async (req, res) => {
    try {
        ...
        if (method === "GET" && pathname === "/new") {
            ...
            if (myHandle && mySecret) {
                ...
                if (p && p.editSecret === mySecret) {
                    return sendHtml(res, layout("作成済み", `<div class="card">
        ...
        if (method === "POST" && pathname === "/new") {
            ...
            if (!name)
                return badRequest(res, "表示名が必要です。");
            const handle = makeUniqueHandle(handleRaw, name);
            if (!makerId)
                return badRequest(res, "職人IDが必要です。");
        ...
        if (method === "POST" && pathname === "/edit") {
            ...
            const tags = parseSelectedTags(params);
            if (!name || !makerId)
                return badRequest(res, "必須項目が足りません。");
            // Top10
            const top10 = [];
            for (let i = 1; i <= 10; i++) {
                const title = (params.get(`c_title_${i}`) ?? "").trim();
                ...
                const note = (params.get(`c_note_${i}`) ?? "").trim();
                if (title || courseId || note)
                    top10.push({ title, courseId, note: note || undefined });
            }
        ...
        return sendHtml(res, layout("サーバエラー", `<p style="color:#c00;">${escapeHtml(String(err?.message ?? err))}</p><p><a href="/">トップへ</a></p>`), 500);
    }
});
function editProfilePage(p) {
    return layout("プロフィール編集", `
<div class="card">
